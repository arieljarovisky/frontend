// CalendarView.jsx (patch de estilos por peluquero + mejor day/week)
  ...
  const styledEvents = useMemo(() => {
    return (filteredEvents || []).map((ev) => {
      ...
      if (st === "pending_deposit") {
        opacity = 0.85;
        pattern = "repeating-linear-gradient(45deg, rgba(255,255,255,.25) 0, rgba(255,255,255,.25) 4px, transparent 4px, transparent 8px)";
    ...
    <div className="bg-white/90 rounded-2xl shadow p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={loadEvents} className="px-3 py-2 rounded-xl border bg-white hover:bg-gray-50 text-sm">Refrescar</button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        {(stylists || []).map((s) => (
          <span key={s.id} className="text-xs px-2 py-1 rounded-full border" style={{borderColor:"#E5E7EB"}}>
            <span className="inline-block w-3 h-3 rounded-sm mr-2" style={{background: s.color_hex?.trim() || stylistColors[s.id] || "#A1A1AA"}} />
            {s.name}
          </span>
        ))}
      </div>
      ...
      <FullCalendar
        ...
        slotDuration="00:15:00"          // más detalle por día
        slotLabelInterval="01:00"        // etiquetas cada hora
        eventMinHeight={26}              // mejor legibilidad
        events={styledEvents}
        datesSet={onDatesSet}
        ...
        eventContent={(arg) => {
          ...
          const stylistName = ep.stylist_name || ep.stylistName || "";
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                {stylistName && (
                  <span
                    style={{
                      ...
                      padding: "2px 6px",
                      borderRadius: 999,
                      background: (ep.stylistColor || "#999") + "22",
                ...
                {status === "pending_deposit" && (
                  <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 999, background: "#FEF3C7", border: "1px solid #FDE68A" }}>
                    seña pendiente
                  </span>
                )}
                {status === "deposit_paid" && (
                  <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 999, background: "#DCFCE7", border: "1px solid #A7F3D0" }}>
                    seña pagada
                  </span>
                )}
                {status === "cancelled" && (
                  <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 999, background: "#F3F4F6", border: "1px solid #E5E7EB" }}>
                    cancelado
                  </span>
                )}
              </div>
